...
import MagazineSubscribeBlock from "@/components/magazine/MagazineSubscribeBlock";
...
export default function CapitalDigest() {
  ...
  const { data: articles = [], isLoading } = useQuery({
    queryKey: ["capital-digest-articles"],
    queryFn: () => loadLatestArticles(PUBLICATION),
    staleTime: 1000 * 60 * 5,
  });

  const generateMutation = useMutation({
    mutationFn: () => generateDailyArticles((p) => setGenerationProgress(p), PUBLICATION),
    onSuccess: () => {
      setGenerationProgress(null);
      queryClient.invalidateQueries({ queryKey: ["capital-digest-articles"] });
    },
    onError: () => setGenerationProgress(null),
  });

  useEffect(() => {
    hasTodaysArticles(PUBLICATION).then((hasToday) => {
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex items-end justify-between gap-4 flex-wrap">
            ...
            <div className="flex items-center gap-3 mb-2">
              ...
              >
                {isGenerating ? <Loader2 className="w-3 h-3 animate-spin mr-1.5" /> : <Zap className="w-3 h-3 mr-1.5" />}
      ...
      <div className="max-w-7xl mx-auto px-4 lg:px-8 py-6">
          ...
          <div className="flex flex-col items-center justify-center py-24 gap-4">
            ...
            <div className="text-center">
              <p className="font-bold text-xl text-black">
                {isGenerating ? "Generating Today's Capital Digest…" : "Loading Capital Digest…"}
              </p>
              <p className="text-slate-500 text-sm font-sans mt-1">
                {isGenerating
                  ? `Searching Bloomberg & Reuters for today's top stories${generationProgress ? ` — writing ${generationProgress.category}` : ""}…`
        ...
        {activeNav === "FRONT PAGE" && heroArticle && (
          <>
            ...
            <div className="grid lg:grid-cols-3 gap-0 border-b border-slate-200 pb-8 mb-8">
              <div className="lg:col-span-2 lg:pr-8 lg:border-r border-slate-200 cursor-pointer group"
                ...
                <p className="text-xs font-sans text-slate-400 tracking-wide">
                  By {heroArticle.author} · {heroArticle.published_date
                    ? format(new Date(heroArticle.published_date), "MMM d, yyyy")
                    : heroArticle.date || ""}
                </p>
              </div>
            ...
            {/* Subscribe Block */}
            <MagazineSubscribeBlock publication={PUBLICATION} />
        ...
        {activeNav !== "FRONT PAGE" && (
          <div>
            ...
            {filtered.length === 0 && !isLoading && !isGenerating && (
              <p className="text-slate-400 font-sans text-sm py-12 text-center">No articles in this category yet.</p>
            )}
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
  ...
  return (
    <div className="cursor-pointer group border-b border-slate-200 pb-5" onClick={onClick}>
      ...
      <p className="text-[10px] font-sans text-slate-400">
        ...
        {!article.published_date && article.date && <> · {article.date}</>}
      </p>
    </div>
  );
}