...
import {
  ...
  Menu, X, LogOut, Briefcase, Building2, UserCog, BadgeDollarSign
} from "lucide-react";
      ...
      <aside className={cn(
        ...
        <div className="flex items-center justify-between p-5 border-b border-slate-100 flex-shrink-0">
          <Link to="/crm/dashboard" className="flex items-center gap-3">
            ...
            <div>
              <h1 className="font-bold text-slate-900 text-sm leading-tight">GLINFICO</h1>
              <p className="text-xs text-amber-600 font-medium">AI-Powered Funding Platform</p>
            </div>
          </Link>
        ...
        <nav className="flex-1 overflow-y-auto p-3 space-y-0.5">
          {navigation.map((item) => {
            ...
            return (
              ...
              >
                <item.icon className={cn("flex-shrink-0", isActive ? "text-amber-600" : "text-slate-400")} style={{ width: '1.1rem', height: '1.1rem' }} />
                {item.name}
              </Link>
            );
          })}
